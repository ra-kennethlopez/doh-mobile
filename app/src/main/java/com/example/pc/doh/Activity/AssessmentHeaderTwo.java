...
public class AssessmentHeaderTwo extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        tooltitle.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(AssessmentPart.path.equals("menu")){
                    Intent part = new Intent(getApplicationContext(), AssessmentPart.class);
                    startActivity(part);
                    Animatoo.animateSlideRight(AssessmentHeaderTwo.this);
                }else {
        ...
        if(checker.checkHasInternet()){
            get_headertwo_online();
        }else{
            get_headertwo_offline();
        ...
        hAdapter.setonItemClickListener(new HeadersAdapter.onItemClickListener() {
            ...
            public void onItemClick(int position) {
                ...
                Intent headerthree = new Intent(getApplicationContext(), AssessmentHeaderThree.class);
                startActivity(headerthree);
                Animatoo.animateSlideLeft(AssessmentHeaderTwo.this);
    ...
    private void get_headertwo_offline(){
        ...
        Cursor det = db.get_item("tbl_assessment_headertwo", "id", AssessmentHeaderOne.id);
                ...
                try {
                    ...
                    if(head.length()>0){
                        for(int i =0;i<head.length();i++){
                            String id = head.getJSONObject(i).getString("id");
                            String desc =head.getJSONObject(i).getString("desc");
                            String hid = db.get_tbl_assessment_header(HomeActivity.appid,uid,id,"2");
                            String assess = "false";
                            if (db.checkDatas("tbl_save_assessment_header", "assessheadid", hid)){
                                countassess++;
                                assess = db.get_tbl_assessment_header_assess(HomeActivity.appid,uid,id,"2");
                            }
    ...
    private void save_assessment_header(){
        String id = db.get_tbl_assessment_header(HomeActivity.appid,uid,AssessmentHeaderOne.id,"1");
...
//            }
            ...
            String[] datas = {AssessmentHeaderOne.id,"1","true",HomeActivity.appid,uid};
    ...
    private void get_headertwo_online(){
        final ProgressBar bar = findViewById(R.id.asesssProgress);
        final ScrollView sv = findViewById(R.id.svheader);
        bar.setVisibility(View.VISIBLE);
        bar.getIndeterminateDrawable().setColorFilter(Color.rgb(40, 165, 95), PorterDuff.Mode.SRC_IN);
        ...
        StringRequest request = new StringRequest(Request.Method.POST, Urls.getheadertwo+HomeActivity.appid+"/"+AssessmentHeaderOne.id,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.d("headerresponse",response);
                        try {
                            JSONObject obj = new JSONObject(response);
                            JSONArray head = obj.getJSONArray("head");
                            int countassess = 0;
                            ArrayList<String> checkid = new ArrayList<>();
                            if (db.checkDatas("tbl_assessment_headertwo", "id", AssessmentHeaderOne.id)){
                                String[] ucolumns = {"json_data"};
                                String[] udata = {response};
                                if (db.update("tbl_assessment_headertwo", ucolumns, udata, "appid",HomeActivity.appid)) {
                                    Log.d("updatedata", "update");
                                } else {
                                    Log.d("updatedata", "not update");
                                }
                                ...
                                String[] datas = {response, uid, HomeActivity.appid,AssessmentHeaderOne.id};
                                if (db.add("tbl_assessment_headertwo", dcolumns, datas, "")) {
                                    Log.d("tbl_headertwo", "added");
                                } else {
                                    Log.d("tbl_headertwo", "not added");
                                }
                            }
                            if(head.length()>0){
                                for(int i =0;i<head.length();i++){
        ...
        expandableListView.setOnGroupClickListener(new ExpandableListView.OnGroupClickListener() {
            ...
            public boolean onGroupClick(ExpandableListView parent, View v, int groupPosition, long id) {
                ...
                switch (headerList.get(groupPosition).index){
                    case 4:
                        Intent main = new Intent(AssessmentHeaderTwo.this,MainActivity.class);
                        startActivity(main);
